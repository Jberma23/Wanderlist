...
#trip
15.times do
Trip.create(start_date: Faker::Date.between(from: 1.year.ago, to: 6.months.ago), end_date: Faker::Date.between(from: 6.months.ago, to: Date.today), user_id: User.all.sample, name:"#{Faker::Address.unique.country} Trip")
Trip.create(start_date: Faker::Date.between(from: Date.today, to: 6.months.from_now), end_date: Faker::Date.between(from: 6.months.from_now, to: 1.year.from_now), user_id: User.all.sample, name:"#{Faker::Address.unique.country} Trip")
end
#flights
15.times do 
Flight.create(flight_number: Faker::Number.number(digits: 5), country_id: Country.all.sample ,trip_id: Trip.all.sample, origin_airport:"#{Faker::Name.unique.initials}" , destination_airport:"#{Faker::Name.unique.initials}")
end

# Activity
15.times do
Activity.create(trip_id: Trip.all.sample, name: "#{Faker::Verb.ing_form}", description: "#{Faker::Lorem.sentences(number: 3)}")
end

15.times do 
Post.create(title:"#{Faker::Lorem.sentences(number: 1)}", content:"#{Faker::Lorem.paragraph(sentence_count: (5))}", user_id: User.all.sample)
end

30.times do 
Like.create(post_id: Post.all.sample, user_id: User.all.sample)
end
